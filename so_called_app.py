...
def count_fee(AID: float, rates: List[Rate]) -> float:
    if find_rate(rates, AID).start is None:
        start = 0
    else:
...
def count_arbitral_tribunal_remuneration(
    ...
    if find_rate(rates, arbitration_fee / 1.06).start is None:
        start = 0
    else:
        start = find_rate(rates, arbitration_fee / 1.06).start
    AT_remuneration = (arbitration_fee / 1.06 - start) * find_rate(
        rates, arbitration_fee / 1.06
    ).rate + find_rate(rates, arbitration_fee / 1.06).base
...
calc_fee = st.checkbox("手动填写仲裁费")

my_placeholder = st.empty()

if calc_fee:
    a_fee = my_placeholder.number_input("手动录入案件仲裁费")
    option1 = st.selectbox("案件类型", case_type)
    number2 = case_type.index(option1) + 1
    number3 = count_arbitral_tribunal_remuneration(
        a_fee, arbitral_tribunal_remuneration_rate
    )
    st.write("仲裁费金额为： ", my_decimal(a_fee))
else:
    number1 = my_placeholder.number_input("争议金额")
    option1 = st.selectbox("案件类型", case_type)
    number2 = case_type.index(option1) + 1
    number3 = count_arbitral_tribunal_remuneration(
...
if expedited_procedure:
    if number2 in (1, 2, 3):
        if my_decimal(number3 * 0.7) <= 10000:
            st.write("独任仲裁员报酬（含裁决稿酬）为： ", my_decimal(10000))
        else:
