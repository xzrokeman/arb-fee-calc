...
# so I wrote this fee_schedule class as a reasonable abstraction of scenarios of 
# this kind.
# example:
...
class fee_schedule:
    ...
    def __post_init__(self):
        ...
        def next_base_generator(n:int, fee_scale:list[float], fee_rate:list[float], base:float) -> float:
            x = 0
            while x < n-1:
                yield base
                x += 1 
        ...
        self.fee_base = [i for i in next_base_generator(len(self.fee_rate),self.fee_scale,self.fee_rate, self.base)]
        self.fee_base.insert(0, self.base)
        self._fee_std = [value for value in range_generator(len(self.fee_scale), self.fee_scale)]
    ...
    def calc(self, AID:Union[int, float]) -> Decimal:
        for std in self.fee_std:            
            if (std.lower_range < AID and std.upper_range >= AID):
                return Decimal(std.base + (AID - std.lower_range) * std.rate).quantize(Decimal("0.00"))
